    ...
    return (
        <DashboardLayout>
            <div className='flex items-center justify-end'>
                <input type="text" placeholder='Buscar paciente...' className="bg-white border border-gray-300 text-gray-900 rounded-lg block w-64 p-2.5"
            ...
            <div>
                ...
                <div className="py-4">
                    {
                        pacientes.length > 0 ?
                         pacientes.filter(paciente => paciente.nombre.toLowerCase().includes(buscarPaciente.toLowerCase())).map((paciente, index) => (
                            <div className='border-b border-gray-100' key={index}>
                                <div className='bg-white hover:bg-gray-100 shadow-sm p-4 flex items-center justify-between rounded-sm my-0.5'>
                                    <div className='flex items-center'>
                                        ...
                                        <p className='text-lg font-bold'>{paciente.nombre} {paciente.primerApellido} {paciente.segundoApellido}</p>
                                    </div>
                                    
                                    <div className='w-12 flex items-center justify-between'>
                                    <div className='cursor-pointer' onClick={()=>router.push('pacientes/'+paciente.id)}>
            ...
            <PopUp
                ...
                popContent={`¿Está seguro de que desea eliminar al paciente ${seleccionarPaciente?.nombre} ${seleccionarPaciente?.primerApellido} ${seleccionarPaciente?.segundoApellido}?`}
                popType="option"
                confirmFunction={() => {
